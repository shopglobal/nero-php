...
public function transfer($amount, $address = '', $payment_id = '', $mixin = 6, $account_index = 0, $subaddr_indices = '', $priority = 2, $unlock_time = 0, $do_not_relay = false)
      ...
      if (array_key_exists('destinations', $params)) {
        ...
        foreach ($destinations as $destination) {
          if (array_key_exists('amount', $destinations[$destination])) {
            $destinations[$destination]['amount'] = $this->$destinations[$destination]['amount'];
          } else {
        ...
        $destinations = array(array('amount' => $this->$amount, 'address' => $address));
      }
  ...
  public function sweep_all($address, $subaddr_indices = '', $account_index = 0, $payment_id = '', $mixin = 6, $priority = 2, $below_amount = 0, $unlock_time = 0, $do_not_relay = false)
  {
    ...
    $params = array('address' => $address, 'mixin' => $mixin, 'get_tx_key' => true, 'subaddr_indices' => $subaddr_indices, 'account_index' => $account_index, 'payment_id' => $payment_id, 'priority' => $priority, 'below_amount' => $this->$below_amount, 'unlock_time' => $unlock_time, 'do_not_relay' => $do_not_relay);
  ...
  public function sweep_single($key_image, $address, $payment_id = '', $mixin = 6, $priority = 2, $below_amount = 0, $unlock_time = 0, $do_not_relay = 0)
  {
    ...
    $params = array('address' => $address, 'mixin' => $mixin, 'get_tx_key' => true, 'account_index' => $account_index, 'payment_id' => $payment_id, 'priority' => $priority, 'below_amount' => $this->$below_amount, 'unlock_time' => $unlock_time, 'do_not_relay' => $do_not_relay);
  ...
  */
  ...
  {
    $params = array('address' => $address, 'amount' => $this->$amount, 'payment_id' => $payment_id, 'recipient_name' => $recipient_name, 'tx_description' => $tx_description);
